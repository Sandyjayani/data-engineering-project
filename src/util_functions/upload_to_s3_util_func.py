...
def upload_tables_to_s3(dataframe: pd.DataFrame, table_name: str,
    ...
    logger.info('Starting upload_tables_to_s3 function.',
    ...
    try:
        ...
        logger.info(f"Table {table_name} has been uploaded to {bucket_name} "
                    f"with key {s3_key}.",
                    extra={
                        'table_name': f"{table_name}",
                        'bucket_name': f"{bucket_name}",
                        's3_key': f"{s3_key}"})
        return f"Table {table_name} has been uploaded to " +\
...
def save_timestamps(table_name: str, timestamp: str, bucket_name: str):
    ...
    try:
        ...
        s3_client.put_object(
                            Bucket=bucket_name,
                            Key=timestamp_key,
                            Body=timestamp_csv_buffer.getvalue())
