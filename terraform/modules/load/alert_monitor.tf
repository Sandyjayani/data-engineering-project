resource "aws_cloudwatch_log_metric_filter" "load_critial_error_filter" {
  name           = "CriticalErrorFilter-Load"
  pattern        = "CRITICAL"
  log_group_name = "/aws/lambda/${var.load_lambda_name}"

  metric_transformation {
    name      = "CriticalErrorCount"
    namespace = "CriticalErrorCount-Load"
    value     = "1"
  }
...
resource "aws_cloudwatch_metric_alarm" "critical_error_alarm" {
  ...
  metric_name               = aws_cloudwatch_log_metric_filter.load_critial_error_filter.metric_transformation[0].name
  namespace                 = aws_cloudwatch_log_metric_filter.load_critial_error_filter.metric_transformation[0].namespace
  period                    = 60
  statistic                 = "Sum"
  threshold                 = 1
