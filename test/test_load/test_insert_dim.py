...
def dim_date():
    return pd.read_parquet(
        "test/test_load/test_data/dim_date-2024-08-23_11.05.48.parquet"
    )
...
def dim_currency():
    return pd.read_parquet(
        "test/test_load/test_data/currency-2024-08-23_10.36.53.parquet"
    )
...
def dim_counterparty():
    return pd.read_parquet(
        "test/test_load/test_data/dim_counterparty-2024-08-23_14.21.28.parquet"
    )
...
def dim_design():
    return pd.read_parquet(
        "test/test_load/test_data/dim_design-2024-08-23_11.41.32.parquet"
    )  # noqa: E501
...
def dim_location():
    return pd.read_parquet(
        "test/test_load/test_data/dim_location-2024-08-23_11.41.33.parquet"
    )  # noqa: E501
...
def dim_staff():
    return pd.read_parquet(
        "test/test_load/test_data/dim_staff-2024-08-23_11.41.36.parquet"
    )
...
def test_successful_insert_date_data(mock_connection, dim_date, caplog):
    ...
    assert (
        list(dim_date.columns) == DIM_DATE_COLUMNS
    ), "Column names do not match for dim_date"
...
def test_successful_insert_dim_counterparty(mock_connection, dim_counterparty, caplog):
    ...
    assert (
        list(dim_counterparty.columns) == DIM_COUNTERPARTY_COLUMNS
    ), "Column names do not match for dim_counterparty"
...
def test_successful_insertion_dim_design(mock_connection, dim_design, caplog):
    ...
    assert (
        list(dim_design.columns) == DIM_DESIGN_COLUMNS
    ), "Column names do not match for dim_design"
...
def test_successful_insertion_dim_location(mock_connection, dim_location, caplog):
    ...
    assert (
        list(dim_location.columns) == DIM_LOCATION_COLUMNS
    ), "Column names do not match for dim_location"
...
def test_successful_insertion_dim_staff(mock_connection, dim_staff, caplog):
    ...
    assert (
        list(dim_staff.columns) == DIM_STAFF_COLUMNS
    ), "Column names do not match for dim_staff"
