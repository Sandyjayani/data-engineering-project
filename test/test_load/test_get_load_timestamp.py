...
def test_get_load_timestamp_success(s3_client):
    table_name = "test_table"
    csv_content = "Date\n2024-08-14_14.09.01\n2024-08-15_15.10.02"
    s3_client.put_object(Bucket="smith-morra-transformation-bucket", Key=f"{table_name}/load_timestamp.csv", Body=csv_content)

    timestamp = get_load_timestamp(table_name)
...
def test_get_load_timestamp_no_csv(s3_client):
...
@patch('src.load.get_load_timestamp.boto3.client')
def test_get_load_timestamp_client_error(mock_boto_client):
...
@patch('src.load.get_load_timestamp.boto3.client')
def test_get_load_timestamp_unexpected_error(mock_boto_client):
    mock_boto_client.return_value.get_object.side_effect = Exception("Unexpected error")
    table_name = "test_table"
    with pytest.raises(Exception, match="Unexpected error"):
        get_load_timestamp(table_name)
