...
def mock_get_load_timestamp():
    with patch('src.load.read_parquet_from_s3.get_load_timestamp') as mock_load_timestamp:
        yield mock_load_timestamp


@pytest.fixture
def mock_get_transformation_timestamp():
    with patch('src.load.read_parquet_from_s3.get_transformation_timestamp') as mock_transformation_timestamp:
...
def test_returns_dictionary(s3, mock_get_transformation_timestamp, mock_get_load_timestamp, mock_pandas, mock_logger):
    mock_get_transformation_timestamp.return_value = datetime(2023, 1, 2, 12, 0, 0)
    mock_get_load_timestamp.return_value = datetime(2023, 1, 1, 12, 0, 0)
    bucket_name = "smith-morra-transformation-bucket"
    table_list = ["dim_date", "dim_location", "dim_design", "dim_currency", "dim_counterparty", "dim_staff", "fact_sales_order"]
...
def test_read_parquet_from_s3_no_new_data(s3, mock_logger, mock_get_transformation_timestamp, mock_get_load_timestamp):
    mock_get_transformation_timestamp.return_value = datetime(2023, 1, 2, 12, 0, 0)
    ...
    mock_logger.info.assert_any_call("No new data to transform for fact_sales_order table.")
...
def test_read_parquet_from_s3_new_data(s3, mock_get_transformation_timestamp, mock_get_load_timestamp, mock_logger, mock_pandas):
