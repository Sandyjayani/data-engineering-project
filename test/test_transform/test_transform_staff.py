...
import pytest
from src.transform.transform_staff import transform_staff as ts, validate_staff_data as vs


@pytest.fixture
def test_df_dict():
    return {'staff': pd.read_csv('test/test_transform/test_data/test_staff.csv'),
            'department': pd.read_csv('test/test_transform/test_data/test_full_department.csv')}
...
class TestTransformStaff:
    ...
    @pytest.mark.it('Calls load_from_bucket if all required departments not in passed department data')
    @patch('src.transform.transform_staff.load_from_bucket')
    def test_calls_load_from_bucket(self, mock_load_from_bucket, part_A_dep_data, part_B_dep_data, test_df_dict):
        test_df_dict['department'] = part_A_dep_data
        mock_load_from_bucket.return_value = part_B_dep_data
        transformed_df = ts(test_df_dict)
        mock_load_from_bucket.assert_called_once()

    @pytest.mark.it('Returns transformed df if no department data is passed in')
    @patch('src.transform.transform_staff.load_from_bucket')
    def test_returns_df_if_not_passed_dep_data(self, mock_load_from_bucket, test_df_dict):
        del test_df_dict['department']
        mock_load_from_bucket.return_value = pd.read_csv('test/test_transform/test_data/test_full_department.csv')
        output_df = ts(test_df_dict)
        expected_cols = ["staff_id", "first_name", "last_name", "department_name", "location", "email_address"]
